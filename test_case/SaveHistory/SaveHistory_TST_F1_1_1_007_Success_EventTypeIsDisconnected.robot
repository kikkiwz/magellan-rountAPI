...
*** Test Cases ***
################### Post ###################
SaveHistory_TST_F1_1_1_007_Success_EventTypeIsDisconnected 
    ...
    #Headers
	${headers}=    Create Dictionary        Content-Type=${HEADER_CONTENT_TYPE}    x-ais-UserName=${HEADER_X_AIS_USERNAME_AISPARTNER}    x-ais-OrderRef=${HEADER_X_AIS_ORDERREF_SAVEHISTORY}${current_timestamp}    x-ais-OrderDesc=${HEADER_X_AIS_ORDERDESC_SAVEHISTORY}    x-ais-AccessToken=Bearer ${accessToken}    x-ais-AccountKey=${AccountId}    Accept=${HEADER_ACCEPT}  
	Log To Console    Headers is : ${headers}
	

	#Body
    ${body}=    Evaluate    {"ThingInfo":{"ThingId":"${ThingIdArr}[0]","ThingName":"${ThingName}","ICCID":"${ICCID}","IMEI":"${IMEI}","QuasarContentType":"${VALUE_SAVEHISTORY_QUASARCONTENTTYPE_REFRESH}","QuasarRefreshTime":"${VALUE_SAVEHISTORY_QUASARREFRESHTIME_0}"},"EventType":"${VALUE_SAVEHISTORY_EVENTTYPE_DISCONNECTED}","Sensors":{"GPS":"${VALUE_SAVEHISTORY_GPS}"},"Timestamp":${current_timestamp}}
    ...
    ${checkReponse}=    Run keyword And Continue On Failure    Response ResultCode Should Have    ${res}    ${SAVESHISTORY}    ${FIELD_OPERATIONSTATUS}    ${FIELD_CODE}    ${FIELD_DEVELOPERMESSAGE}
